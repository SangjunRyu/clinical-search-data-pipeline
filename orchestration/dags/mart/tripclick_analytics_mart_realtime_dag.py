"""
TripClick Analytics Mart Realtime DAG (Hot Mart)

- 목적:
  Curated Stream → PostgreSQL 실시간 마트 적재 (Near Real-Time)
- 구성:
  - SSHOperator로 Spark 서버에서 Structured Streaming 실행
- 특징:
  - 1~5분 마이크로배치로 Hot Analytics Mart 생성
  - 4개 실시간 마트 동시 적재:
...
# =========================
with DAG(
    dag_id="tripclick_analytics_mart_realtime",
    description="TripClick Curated Stream → PostgreSQL 실시간 마트 (Hot Mart) DAG",
    default_args=DEFAULT_ARGS,
    start_date=datetime(2026, 1, 1),
    schedule_interval=None,  # 수동 실행 (장기 실행 Job)
    catchup=False,
    tags=["tripclick", "analytics_mart", "realtime", "streaming", "hot"],
    doc_md="""
    ## TripClick Analytics Mart Realtime DAG

    ### 목적
    Curated Stream 데이터를 1~5분 마이크로배치로 읽어 Hot Analytics Mart를 PostgreSQL에 적재합니다.
    ...
    ### 필요 설정
    - Variables: `S3_CURATED_STREAM_PATH`, `S3_CHECKPOINT_PATH`
    ...
    # =========================
    # Streaming to Analytics Mart Realtime (via SSH)
    # =========================
    streaming_to_analytics_mart_realtime = SSHOperator(
        task_id="streaming_to_analytics_mart_realtime",
        ssh_conn_id=SPARK_SSH_CONN_ID,
        command=f"""
docker exec \\
  -e S3_CURATED_STREAM_PATH="{S3_CURATED_STREAM_PATH}" \\
  ...
  /opt/spark/jobs/streaming_to_analytics_mart_realtime.py
""",
